...
import { User, Building, ChevronRight, FileText, Globe, Link as LinkIcon, FileSearch, Quote, Network, Menu, X } from 'lucide-react'
      ...
      <div className="fixed w-full top-0 z-50 shadow-md">
        ...
        <header className="bg-gov-ivory/95 backdrop-blur-md px-6 md:px-12 h-20 flex items-center justify-between border-b border-gov-light">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-serif-official font-bold tracking-widest text-gov-blue leading-none uppercase">
                Root-Claim
              </h1>
              <p className="text-[7px] md:text-[9px] uppercase tracking-[0.2em] text-gov-gold mt-1 font-bold">
          ...
          <button className="lg:hidden text-gov-blue" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="lg:hidden bg-white border-b border-gov-light absolute w-full top-20 left-0 shadow-xl flex flex-col p-6 gap-6">
            <nav className="flex flex-col gap-4 text-sm font-bold tracking-widest text-gov-blue uppercase">
              <a href="#about" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-gov-gold transition-colors">About Mission</a>
              <a href="#services" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-gov-gold transition-colors">Citizen Services</a>
              <a href="#architecture" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-gov-gold transition-colors">Architecture</a>
            </nav>
            <div className="flex flex-col gap-3">
              <Link href="/login" onClick={() => setIsMobileMenuOpen(false)}>
                <button className="w-full bg-gov-gold text-white px-5 py-3 text-xs font-bold tracking-widest uppercase hover:bg-[#9a7b3b] transition-colors shadow-sm">
                  Citizen Portal
                </button>
              </Link>
              <Link href="/admin-login" onClick={() => setIsMobileMenuOpen(false)}>
                <button className="w-full bg-gov-blue text-white px-5 py-3 text-xs font-bold tracking-widest uppercase hover:bg-[#081729] transition-colors shadow-sm">
                  Ministry Login
                </button>
              </Link>
            </div>
      ...
      <section id="main-content" className="bg-gov-blue relative text-white border-b-4 border-gov-gold overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 py-24 md:py-32 relative z-10 flex flex-col md:flex-row items-center gap-12">
          <div className="flex-1">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link href="/login" className="w-full sm:w-auto">
                <button className="w-full bg-gov-gold text-white px-8 py-3.5 text-xs font-bold tracking-widest uppercase hover:bg-white hover:text-gov-blue transition-colors flex items-center justify-center gap-2">
                  Submit a Claim <ChevronRight size={16} />
                </button>
              </Link>
              <Link href="/login" className="w-full sm:w-auto">
                <button className="w-full bg-transparent border border-white text-white px-8 py-3.5 text-xs font-bold tracking-widest uppercase hover:bg-white hover:text-gov-blue transition-colors">
          ...
          <div className="hidden md:flex w-96 h-96 bg-gov-ivory/5 border border-white/10 rounded-full items-center justify-center relative shadow-2xl backdrop-blur-sm">
            ...
            <Logo type="citizen" size={240} className="shadow-[0_0_50px_rgba(234,179,8,0.3)]" />
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs text-gray-600 font-semibold">
              ...
              <li><Link href="/login" className="hover:text-gov-gold transition-colors flex items-center gap-2"><FileText size={12}/> Submit a Claim</Link></li>
            </ul>
          </div>
